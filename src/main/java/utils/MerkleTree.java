...
public class MerkleTree {
    ...
    public MerkleTree(byte[][] leafs, String digestAlgorithm) {
        ...
        for (int i = depth - 1; i >= 1; i--) {
            for (int j = (1 << depth - 1) - 1; j < (1 << depth) - 1; j++) {
    ...
    public byte[][] getProof(int index) {
        ...
        byte[][] proof = new byte[depth - 1][256];

        // eg.对于深度为4的merkle树，叶节点索引0~7，对应到tree中的索引就是7~14，因为tree中包含父节点，需要转换以下索引
